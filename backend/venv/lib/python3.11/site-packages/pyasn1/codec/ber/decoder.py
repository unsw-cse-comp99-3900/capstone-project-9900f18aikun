...
class AbstractPayloadDecoder(object):
    ...
    def valueDecoder(self, substrate, asn1Spec,
        ...
        raise error.PyAsn1Error(
            'SingleItemDecoder not implemented for %s' %
...
class NullPayloadDecoder(AbstractSimplePayloadDecoder):
    ...
    def valueDecoder(self, substrate, asn1Spec,
        ...
        if chunk:
            raise error.PyAsn1Error(
                'Unexpected %d-octet substrate for Null' %
                length)
    ...
    def __call__(self, substrate, asn1Spec=None,
        ...
        while state is not stStop:
                    ...
                    if tagId == 0x1F:
                        ...
                        while True:
                            for integerByte in readFromStream(
                                    substrate, 1, options):
                                if isinstance(
                                        integerByte, SubstrateUnderrunError):
                                    yield integerByte
            ...
            if state is stGetValueDecoderByAsn1Spec:
                ...
                if chosenSpec is not None:
                    try:
                        ...
                        # use base type for codec lookup to recover untagged
                        # types
            ...
            if state is stDecodeValue:
                if not options.get('recursiveFlag',
                                   True) and not substrateFun:  # deprecate this
                    def substrateFun(
                ...
                if length == -1:  # indef length
                    ...
                    if not substrateFun and bytesRead != length:
                        ...
                        # custom substrateFun may be used for partial decoding,
                        # reading less is expected there
                        raise PyAsn1Error(
            ...
            def substrateFunWrapper(
                ...
                try:
                    ...
                    if traceback.tb_next:
                        # Traceback depth > 1 means TypeError from inside user
                        # provided function
                        raise
                    # invariant maintained at Decoder.__call__ entry
                    assert isinstance(
                        substrate, io.BytesIO)  # nosec assert_used
