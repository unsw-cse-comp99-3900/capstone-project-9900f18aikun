...
def confloat(
    *,
    ...
    return Annotated[float,
                     Strict(strict) if strict is not None else None,
                     annotated_types.Interval(gt=gt,
                                              ge=ge,
                                              lt=lt,
                                              le=le),
                     annotated_types.MultipleOf(multiple_of) if multiple_of is not None else None,
...
def conset(item_type: type[HashableItemType],
           *,
           min_length: int | None = None,
           max_length: int | None = None) -> type[set[HashableItemType]]:
    ...
    """
    return Annotated[Set[item_type], annotated_types.Len(
        min_length or 0, max_length)]
...
def confrozenset(
    ...
    """
    return Annotated[FrozenSet[item_type],
                     annotated_types.Len(min_length or 0, max_length)]
...
def conlist(
    ...
    *,
    ...
    return Annotated[List[item_type],
                     annotated_types.Len(min_length or 0, max_length)]
    ...
    *,
    ...
    return Annotated[Decimal,
                     ...
                     annotated_types.Interval(gt=gt,
                                              ...
                                              le=le),
                     annotated_types.MultipleOf(multiple_of) if multiple_of is not None else None,
                     _fields.pydantic_general_metadata(max_digits=max_digits,
...
class _SecretBase(Generic[SecretType]):
    ...
    def __eq__(self, other: Any) -> bool:
        return isinstance(
            other, self.__class__) and self.get_secret_value() == other.get_secret_value()
...
class Secret(_SecretBase[SecretType]):
    ...
    def __get_pydantic_core_schema__(
        ...
        def serialize(
                ...
                info: core_schema.SerializationInfo) -> str | Secret[SecretType]:
            if info.mode == 'json':
                return str(value)
            else:
                return value
...
class _SecretField(_SecretBase[SecretType]):
    ...
    def __get_pydantic_core_schema__(
        ...
        def serialize(
                ...
                info: core_schema.SerializationInfo) -> str | _SecretField[SecretType]:
            if info.mode == 'json':
...
class ByteSize(int):
    ...
    byte_sizes.update({k.lower()[0]: v for k,
                       v in byte_sizes.items() if 'i' not in k})
    ...
    def _validate(cls, input_value: Any, /                  , _: core_schema.ValidationInfo) -> ByteSize:
        ...
        try:
            ...
            raise PydanticCustomError(
                'byte_size_unit', 'could not interpret byte unit: {unit}', {
                    'unit': unit})
...
class Base64Encoder(EncoderProtocol):
    ...
    def decode(cls, data: bytes) -> bytes:
        ...
        try:
            ...
            raise PydanticCustomError(
                'base64_decode', "Base64 decoding error: '{error}'", {
                    'error': str(e)})
...
class Discriminator:
    ...
    def __get_pydantic_core_schema__(
        ...
        if isinstance(self.discriminator, str):
            ...
            return handler(Annotated[source_type,
    ...
    def _convert_schema(
            self,
            original_schema: core_schema.CoreSchema) -> core_schema.TaggedUnionSchema:
