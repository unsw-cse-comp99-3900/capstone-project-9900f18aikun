...
class PydanticPlugin(Plugin):
    ...
    def get_base_class_hook(
            self, fullname: str) -> Callable[[ClassDefContext], bool] | None:
    ...
    def get_metaclass_hook(
            self, fullname: str) -> Callable[[ClassDefContext], None] | None:
    ...
    def get_function_hook(
            self, fullname: str) -> Callable[[FunctionContext], Type] | None:
    ...
    def get_method_hook(
            self, fullname: str) -> Callable[[MethodContext], Type] | None:
            ...
            if isinstance(default_factory_type, CallableType):
                ...
                if args:
                    if all(isinstance(arg, TypeVarType) for arg in args):
                        ...
                        ret_type.args = tuple(
                            default_any_type for _ in args)  # type: ignore[attr-defined]
                return ret_type
...
class PydanticModelField:
    ...
    def to_argument(
        ...
        type_annotation = self.expand_type(
            current_info, api) if typed else AnyType(
            TypeOfAny.explicit)
        return Argument(
            variable=variable,
    ...
    def expand_type(
            self,
            current_info: TypeInfo,
            api: SemanticAnalyzerPluginInterface) -> Type | None:
        """Based on mypy.plugins.dataclasses.DataclassAttribute.expand_type."""
        # The getattr in the next line is used to prevent errors in legacy
        # versions of mypy without this attribute
        if self.type is not None and getattr(
                self.info, 'self_type', None) is not None:
    ...
    def deserialize(
            ...
            info: TypeInfo,
            data: JsonDict,
            api: SemanticAnalyzerPluginInterface) -> PydanticModelField:
...
class PydanticModelTransformer:
    ...
    def transform(self) -> bool:
        ...
        * stores the fields, config, and if the class is settings in the mypy metadata for access by subclasses
        ...
        info.metadata[METADATA_KEY] = {
            'fields': {
                field.name: field.serialize() for field in fields},
            'class_vars': {
        ...
        for stmt in cls.defs.body:
            ...
            if isinstance(stmt, AssignmentStmt):
                ...
                if isinstance(stmt.rvalue,
                              CallExpr):  # calls to `dict` or `ConfigDict`
    ...
    def collect_fields_and_class_vars(
        ...
        for info in reversed(
                cls.info.mro[1:-1]):  # 0 is the current class, -2 is BaseModel, -1 is object
            ...
            for name, data in info.metadata[METADATA_KEY]['fields'].items():
                ...
                if sym_node and sym_node.node and not isinstance(
                        sym_node.node, Var):
                    self._api.fail(
    ...
    def _get_assignment_statements_from_if_statement(
            self, stmt: IfStmt) -> Iterator[AssignmentStmt]:
    ...
    def _get_assignment_statements_from_block(
            self, block: Block) -> Iterator[AssignmentStmt]:
    ...
    def collect_field_or_class_var_from_stmt(  # noqa C901
        ...
        lhs = stmt.lvalues[0]
        if not isinstance(
                lhs, NameExpr) or not _fields.is_valid_field_name(
                lhs.name) or lhs.name == 'model_config':
            return None
        ...
        if not _fields.is_valid_field_name(
                lhs.name) or lhs.name == 'model_config':
            return None
        ...
        if isinstance(
                node_type,
                Instance) and node_type.type.fullname == 'dataclasses.InitVar':
            self._api.fail(
    ...
    def _infer_dataclass_attr_init_type(
        ...
        if setter:
            if isinstance(setter.node, FuncDef):
                ...
                if isinstance(
                    setter_type,
                    CallableType) and setter_type.arg_kinds == [
                    ARG_POS,
                    ARG_POS,
                    ARG_POS,
                ]:
    ...
    def add_initializer(
        ...
        if is_root_model and MYPY_VERSION_TUPLE <= (1, 0, 1):
            ...
            # This is needed because mypy support for `dataclass_transform`
            # isn't complete on 1.0.1
                ...
                if base_settings_init_node is not None and base_settings_init_node.type is not None:
                    ...
                    for arg_idx, arg_name in enumerate(func_type.arg_names):
                        if arg_name.startswith(
                                '__') or not arg_name.startswith('_'):
                            continue
    ...
    def get_config_update(
            ...
            name: str,
            arg: Expression,
            lax_extra: bool = False) -> ModelConfigData | None:
        ...
        if isinstance(
            ...
            for arg, name in zip(expr.args, expr.arg_names):
                ...
                if name == 'default_factory':
                    return not (isinstance(arg, NameExpr)
    ...
    def should_init_forbid_extra(self,
                                 fields: list[PydanticModelField],
...
ERROR_UNTYPED = ErrorCode(
    'pydantic-field',
    'Untyped field disallowed',
    'Pydantic')
ERROR_FIELD_DEFAULTS = ErrorCode(
    'pydantic-field',
    'Invalid Field defaults',
    'Pydantic')
