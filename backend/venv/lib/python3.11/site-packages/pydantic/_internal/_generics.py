...
def get_model_typevars_map(
        cls: type[BaseModel]) -> dict[TypeVarType, Any] | None:
...
def replace_types(type_: Any, type_map: Mapping[Any, Any] | None) -> Any:
    ...
    if type_args:
        ...
        return origin_type[resolved_type_args[0] if len(
            resolved_type_args) == 1 else resolved_type_args]
...
def has_instance_in_type(type_: Any, isinstance_target: Any) -> bool:
    ...
    # `typing.Callable[[int, str], int]` is an example for this.
    if isinstance(
            type_, (List, list)) and not isinstance(
            type_, typing_extensions.ParamSpec):
        if any(has_instance_in_type(element, isinstance_target)
               for element in type_):
            return True
...
def check_parameters_count(
        cls: type[BaseModel], parameters: tuple[Any, ...]) -> None:
...
def get_cached_generic_type_early(
        ...
        typevar_values: Any) -> type[BaseModel] | None:
