...
def get_enum_core_schema(
    ...
    if cases:
        ...
        def get_json_schema(schema: CoreSchema,
                            handler: GetJsonSchemaHandler) -> JsonSchemaValue:
        ...
        # we don't want to add the missing to the schema if it's the default
        # one
        default_missing = getattr(
            enum_type._missing_,
            '__func__',
            None) == Enum._missing_.__func__  # type: ignore
        ...
        def get_json_schema_no_cases(
                _, handler: GetJsonSchemaHandler) -> JsonSchemaValue:
            json_schema = handler(
...
class InnerSchemaValidator:
    ...
    def __get_pydantic_core_schema__(
            self,
            _source_type: Any,
            _handler: GetCoreSchemaHandler) -> CoreSchema:
...
def uuid_prepare_pydantic_annotations(
    ...
    # UUIDs have no constraints - they are fixed length, constructing a UUID
    # instance checks the length
    ...
    return (
        source_type, [
            InnerSchemaValidator(
                core_schema.uuid_schema()), *annotations])
...
def path_schema_prepare_pydantic_annotations(
    ...
    return (
        source_type, [
            InnerSchemaValidator(
                schema, js_core_schema=constrained_str_schema, js_schema_update={
                    'format': 'path'}), *remaining_annotations, ], )


def dequeue_validator(input_value: Any,
                      handler: core_schema.ValidatorFunctionWrapHandler,
                      maxlen: None | int) -> collections.deque[Any]:
...
def serialize_sequence_via_list(
        ...
        info: core_schema.SerializationInfo) -> Any:
    items: list[Any] = []
        ...
        if self.mapped_origin in (list, set, frozenset):
            ...
            if self.mapped_origin is collections.deque:
                ...
                coerce_instance_wrap = partial(
                    core_schema.no_info_wrap_validator_function, partial(
                        dequeue_validator, maxlen=metadata.get(
                            'max_length', None)), )
            else:
                coerce_instance_wrap = partial(
...
def sequence_like_prepare_pydantic_annotations(
    ...
    mapped_origin = SEQUENCE_ORIGIN_MAP.get(
        origin, None) if origin else SEQUENCE_ORIGIN_MAP.get(
        source_type, None)
    if mapped_origin is None:
        return None
    ...
    return (
        source_type, [
            SequenceValidator(
                mapped_origin, item_source_type, **metadata), *remaining_annotations])
...
def defaultdict_validator(input_value: Any,
                          handler: core_schema.ValidatorFunctionWrapHandler,
                          default_default_factory: Callable[[],
                                                            Any]) -> collections.defaultdict[Any,
                                                                                             Any]:
...
def get_defaultdict_default_default_factory(
        values_source_type: Any) -> Callable[[], Any]:
    ...
    if _typing_extra.is_annotated(values_source_type):
        field_info = next(
            (v for v in get_args(values_source_type) if isinstance(
                v, FieldInfo)), None)
    else:
        field_info = None
...
class MappingValidator:
    ...
    def serialize_mapping_via_dict(
            self,
            v: Any,
            handler: core_schema.SerializerFunctionWrapHandler) -> Any:
        return handler(v)
...
def mapping_like_prepare_pydantic_annotations(
    ...
    mapped_origin = MAPPING_ORIGIN_MAP.get(
        origin, None) if origin else MAPPING_ORIGIN_MAP.get(
        source_type, None)
    if mapped_origin is None:
        return None
    ...
    return (source_type,
            [MappingValidator(mapped_origin,
                              keys_source_type,
                              values_source_type,
                              **metadata),
