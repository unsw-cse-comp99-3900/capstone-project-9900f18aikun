...
def filter_field_decorator_info_by_field(
    ...
    return [
        dec for dec in validator_functions if check_validator_fields_against_field_name(
            dec.info,
            field)]
...
def apply_each_item_validators(
    ...
    if schema['type'] == 'nullable':
        ...
        if (variadic_item_index := schema.get(
                'variadic_item_index')) is not None:
...
def modify_model_json_schema(
    ...
    # Preserve the fact that definitions schemas should never have sibling
    # keys:
    ...
    # BaseModel; don't use cls.__doc__ as it will contain the verbose class
...
class GenerateSchema:
    ...
    def _dict_schema(self, tp: Any, keys_type: Any,
                     values_type: Any) -> CoreSchema:
    ...
    def _add_js_function(self, metadata_schema: CoreSchema,
                         js_function: Callable[..., Any]) -> None:
            ...
            if core_config.get('extra_fields_behavior') == 'allow':
                ...
                for candidate_cls in cls.__mro__[:-1]:
                    extras_annotation = getattr(
                        candidate_cls, '__annotations__', {}).get(
                        '__pydantic_extra__', None)
                    if extras_annotation is not None:
                        if isinstance(extras_annotation, str):
                            extras_annotation = _typing_extra.eval_type_backport(
                                _typing_extra._make_forward_ref(
                                    extras_annotation, is_argument=False, is_class=True), self._types_namespace, )
                        ...
                        extra_items_type = self._get_args_resolving_forward_refs(
                            extras_annotation, required=True, )[1]
    ...
    def _generate_schema_from_property(
            self, obj: Any, source: Any) -> core_schema.CoreSchema | None:
        ...
        if (get_schema := getattr(obj,
            ...
            if len(inspect.signature(get_schema).parameters) == 1:
                ...
                schema = get_schema(source, CallbackGetCoreSchemaHandler(
    ...
    def _get_args_resolving_forward_refs(
            self, obj: Any, required: bool = False) -> tuple[Any, ...] | None:
        args = get_args(obj)
        if args:
    ...
    def _common_field_schema(  # C901
        ...
        json_schema_updates = {
            ...
            'deprecated': bool(
                field_info.deprecated) or field_info.deprecated == '' or None,
            'examples': to_jsonable_python(
                field_info.examples),
        }
        json_schema_updates = {
            k: v for k,
    ...
    def _typed_dict_schema(
            self,
            typed_dict_cls: Any,
            origin: Any) -> core_schema.CoreSchema:
        ...
        with self.model_type_stack.push(typed_dict_cls), self.defs.get_schema_or_ref(typed_dict_cls) as (
            ...
            if not _SUPPORTS_TYPEDDICT and type(
                    typed_dict_cls).__module__ == 'typing':
                raise PydanticUserError(
            ...
            with self._config_wrapper_stack.push(config), self._types_namespace_stack.push(typed_dict_cls):
                ...
                for field_name, annotation in get_type_hints_infer_globalns(
                        typed_dict_cls, localns=self._types_namespace, include_extras=True).items():
                    annotation = replace_types(annotation, typevars_map)
                ...
                [
                    self._generate_parameter_schema(
                        ...
                        default=namedtuple_cls._field_defaults.get(
                            field_name,
                            Parameter.empty)) for field_name,
    ...
    def _generate_parameter_schema(self,
        ...
        if field.alias is not None:
            ...
            if isinstance(
                    alias_generator,
                    AliasGenerator) and alias_generator.alias is not None:
    ...
    def _union_is_subclass_schema(
            self, union_type: Any) -> core_schema.CoreSchema:
    ...
    def _subclass_schema(self, type_: Any) -> core_schema.CoreSchema:
        ...
        if type_param == Any:
            ...
            if type_param.__bound__:
                ...
                return core_schema.union_schema([self.generate_schema(
                    typing.Type[c]) for c in type_param.__constraints__])
            else:
                return self._type_schema()
        ...
        metadata = build_metadata_dict(
            js_functions=[
                ...
                _2: {
                    'type': 'string',
                    'format': 'regex'}])
        ser = core_schema.plain_serializer_function_ser_schema(attrgetter(
            'pattern'), when_used='json', return_schema=core_schema.str_schema())
    ...
    def _dataclass_schema(
            ...
            origin: type[StandardDataclass] | None) -> core_schema.CoreSchema:
        ...
        with self.model_type_stack.push(dataclass), self.defs.get_schema_or_ref(dataclass) as (
            ...
            with ExitStack() as dataclass_bases_stack:
                # Pushing a namespace prioritises items already in the stack,
                # so iterate though the MRO forwards
                ...
                # Pushing a config overwrites the previous config, so iterate
                # though the MRO backwards
                ...
                # disallow combination of init=False on a dataclass field and
                # extra='allow' on a dataclass
                    ...
                    for field_name, field in fields.items():
                        if field.init is False:
                            raise PydanticUserError(
                                ...
                                f'This combination is not allowed.', code='dataclass-init-false-extra-allow', )
                ...
                args = sorted(
                    (self._generate_dc_field_schema(
                        k,
                        v,
                        decorators) for k,
                        v in fields.items()),
    ...
    def _callable_schema(self,
                         function: Callable[...,
                                            Any]) -> core_schema.CallSchema:
        ...
        mode_lookup: dict[_ParameterKind,
                          Literal['positional_only',
                                  'positional_or_keyword',
                                  'keyword_only']] = {Parameter.POSITIONAL_ONLY: 'positional_only',
    ...
    def _unsubstituted_typevar_schema(
            self, typevar: typing.TypeVar) -> core_schema.CoreSchema:
        ...
        try:
            ...
            typevar_has_default = getattr(
                typevar, '__default__', None) is not None
        ...
        if typevar_has_default:
            ...
            return self._union_schema(
                typing.Union[constraints])  # type: ignore
        elif bound:
    ...
    def _computed_field_schema(
        ...
        return_type_schema = self._apply_field_serializers(
            return_type_schema, filter_field_decorator_info_by_field(
                field_serializers.values(), d.cls_var_name), computed_field=True, )
    ...
    def _apply_single_annotation(
            self,
            schema: core_schema.CoreSchema,
            metadata: Any) -> core_schema.CoreSchema:
        from ..fields import FieldInfo
        ...
        if isinstance(metadata, FieldInfo):
            ...
            if json_schema_update or json_schema_extra:
                CoreMetadataHandler(schema).metadata.setdefault(
                    'pydantic_js_annotation_functions', []).append(
                    get_json_schema_update_func(
    ...
    def _get_wrapped_inner_schema(
        ...
        metadata_get_schema: GetCoreSchemaFunction = getattr(
            annotation, '__get_pydantic_core_schema__', None) or (
...
_VALIDATOR_F_MATCH: Mapping[tuple[FieldValidatorModes,
                            ...
                            Callable[[Callable[...,
                                  ...
                                  'no-info'): lambda f,
                                 schema,
                                 _: core_schema.no_info_before_validator_function(f,
                                                                                  schema),
                                 ('after',
                                  'no-info'): lambda f,
                                 schema,
                                 _: core_schema.no_info_after_validator_function(f,
                                                                                 schema),
                                 ('plain',
                                  'no-info'): lambda f,
                                 _1,
                                 ...
                                 ('wrap',
                                  'no-info'): lambda f,
                                 schema,
                                 _: core_schema.no_info_wrap_validator_function(f,
                                                                                schema),
                                 ('before',
                                  'with-info'): lambda f,
                                 schema,
                                 field_name: core_schema.with_info_before_validator_function(f,
                                                                                             schema,
                                                                                             field_name=field_name),
                                 ('after',
                                  'with-info'): lambda f,
                                 schema,
                                 field_name: core_schema.with_info_after_validator_function(f,
                                                                                            schema,
                                                                                            field_name=field_name),
                                 ('plain',
                                  'with-info'): lambda f,
                                 ...
                                 ('wrap',
                                  'with-info'): lambda f,
                                 schema,
                                 field_name: core_schema.with_info_wrap_validator_function(f,
                                                                                           schema,
                                                                                           field_name=field_name),
                                 }
...
def wrap_default(
        field_info: FieldInfo,
        schema: core_schema.CoreSchema) -> core_schema.CoreSchema:
...
def _extract_get_pydantic_json_schema(
        tp: Any, schema: CoreSchema) -> GetJsonSchemaFunction | None:
...
def get_json_schema_update_func(json_schema_update: JsonSchemaValue,
                                json_schema_extra: JsonDict | typing.Callable[[JsonDict],
                                                                              None] | None) -> GetJsonSchemaFunction:
...
def add_json_schema_extra(json_schema: JsonSchemaValue,
                          json_schema_extra: JsonDict | typing.Callable[[JsonDict],
                                                                        None] | None):
...
class _Definitions:
    ...
    def get_schema_or_ref(
            self, tp: Any) -> Iterator[tuple[str, None] | tuple[str, CoreSchema]]:
...
def resolve_original_schema(schema: CoreSchema,
                            definitions: dict[str,
                                              CoreSchema]) -> CoreSchema | None:
