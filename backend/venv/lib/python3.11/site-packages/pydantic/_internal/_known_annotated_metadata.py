...
def add_js_update_schema(
        s: cs.CoreSchema, f: Callable[[], dict[str, Any]]) -> None:
    def update_js_schema(
            s: cs.CoreSchema, handler: GetJsonSchemaHandler) -> dict[str, Any]:
        js_schema = handler(s)
        js_schema.update(f())
        return js_schema
...
def apply_known_metadata(annotation: Any, schema: CoreSchema) -> CoreSchema | None:  # noqa: C901
    ...
    for constraint, value in schema_update.items():
        ...
        # in this recursive case with function-after or function-wrap, we
        # should refactor
        if schema_type in {
            'function-before',
            'function-wrap',
                'function-after'} and constraint == 'strict':
            # type: ignore  # schema is function-after schema
...
def collect_known_metadata(
        annotations: Iterable[Any]) -> tuple[dict[str, Any], list[Any]]:
...
def check_metadata(metadata: dict[str,
                                  Any],
