...
class PydanticDescriptorProxy(Generic[ReturnType]):
    ...
    def _call_wrapped_attr(self,
                           func: Callable[[Any],
                                          None],
                           *,
                           name: str) -> PydanticDescriptorProxy[ReturnType]:
    ...
    def __get__(
            self,
            obj: object | None,
            obj_type: type[object] | None = None) -> PydanticDescriptorProxy[ReturnType]:
        try:
        ...
        *,
        ...
        if not callable(func):
            ...
            attribute = get_attribute_from_base_dicts(
                cls_, cls_var_name)  # prevents the binding call to `__get__`
    ...
    def merge_seqs(seqs: list[deque[type[Any]]]) -> Iterable[type[Any]]:
        while True:
            ...
            for seq in non_empty:  # Find merge candidates among seq heads.
                candidate = seq[0]
                not_head = [
                    s for s in non_empty if candidate in islice(
                        s, 1, None)]
                if not_head:
...
def get_attribute_from_bases(
        tp: type[Any] | tuple[type[Any], ...], name: str) -> Any:
                        ...
                        for f in info.fields:
                            if f in field_serializer_decorator.info.fields:
                                raise PydanticUserError(
                                    ...
                                    f'for field {f!r}, this is not allowed.', code='multiple-field-serializers', )
...
def inspect_validator(
        validator: Callable[..., Any], mode: FieldValidatorModes) -> bool:
...
def inspect_annotated_serializer(
        serializer: Callable[..., Any], mode: Literal['plain', 'wrap']) -> bool:
...
def inspect_model_serializer(
    ...
    """
    if isinstance(serializer, (staticmethod, classmethod)
...
def _serializer_info_arg(
        mode: Literal['plain', 'wrap'], n_positional: int) -> bool | None:
    if mode == 'plain':
        if n_positional == 1:
            # (input_value: Any, /) -> Any
            return False
...
def ensure_classmethod_based_on_signature(
        function: AnyDecoratorCallable) -> Any:
    ...
    if not isinstance(
            unwrap_wrapped_function(
                function,
                unwrap_class_static_method=False),
            classmethod) and _is_classmethod_from_sig(function):
...
def get_function_return_type(func: Any,
