...
def _field_name_for_signature(field_name: str, field_info: FieldInfo) -> str:
    ...
    return _alias_if_valid(
        field_info.alias) or _alias_if_valid(
        field_info.validation_alias) or field_name
...
def _process_param_defaults(param: Parameter) -> Parameter:
    ...
    if isinstance(param_default, FieldInfo):
        ...
        return param.replace(
            ...
            name=_field_name_for_signature(
                param.name,
                param_default),
            default=default)
    return param
...
def generate_pydantic_signature(init: Callable[...,
                                               None],
                                fields: dict[str,
                                             FieldInfo],
                                config_wrapper: ConfigWrapper,
                                is_dataclass: bool = False) -> Signature:
    ...
    if is_dataclass:
        merged_params = {
            k: _process_param_defaults(v) for k,
            v in merged_params.items()}
