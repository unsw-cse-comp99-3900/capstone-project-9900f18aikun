...
class _ModelNamespaceDict(dict):
    ...
    def __setitem__(self, k: str, v: object) -> None:
        ...
        if existing and v is not existing and isinstance(
                existing, PydanticDescriptorProxy):
            warnings.warn(
...
@dataclass_transform(kw_only_default=True,
                     field_specifiers=(PydanticModelField,))
class ModelMetaclass(ABCMeta):
        ...
        if bases:
            ...
            if Generic in mro and mro.index(Generic) < mro.index(BaseModel):
                warnings.warn(
                    GenericBeforeBaseModelWarning(
                        ...
                        'for pydantic generics to work properly.'), stacklevel=2, )
            ...
            if __pydantic_generic_metadata__:
                ...
                parent_parameters = getattr(
                    cls, '__pydantic_generic_metadata__', {}).get(
                    'parameters', ())
                parameters = getattr(
                    cls, '__parameters__', None) or parent_parameters
                if parameters and parent_parameters and not all(
                        x in parameters for x in parent_parameters):
            ...
            cls.model_computed_fields = {
                k: v.info for k,
                v in cls.__pydantic_decorators__.computed_fields.items()}
            ...
            super(
                cls, cls).__pydantic_init_subclass__(
                **kwargs)  # type: ignore[misc]
            return cls
        else:
            # this is the BaseModel class itself being created, no logic
            # required
    ...
    if not typing.TYPE_CHECKING:  # pragma: no branch
        ...
        def __getattr__(self, item: str) -> Any:
            ...
            if item == '__pydantic_core_schema__':
                # This means the class didn't get a schema generated for it,
                # likely because there was an undefined reference
                ...
                if isinstance(maybe_mock_validator, MockValSer):
                    ...
                    if rebuilt_validator is not None:
                        # In this case, a validator was built, and so
                        # `__pydantic_core_schema__` should now be set
    ...
    def __instancecheck__(self, instance: Any) -> bool:
        ...
        return hasattr(
            instance,
            '__pydantic_validator__') and super().__instancecheck__(instance)

    @staticmethod
    def _collect_bases_data(
            bases: tuple[type[Any], ...]) -> tuple[set[str], set[str], dict[str, ModelPrivateAttr]]:
        from ..main import BaseModel
...
def get_model_post_init(
        namespace: dict[str, Any], bases: tuple[type[Any], ...]) -> Callable[..., Any] | None:
    ...
    for ann_name, ann_type in raw_annotations.items():
        if (
            ...
            if is_annotated(ann_type):
                ...
                private_attr = next(
                    (v for v in metadata if isinstance(
                        v, ModelPrivateAttr)), None)
...
def set_default_hash_func(
        cls: type[BaseModel], bases: tuple[type[Any], ...]) -> None:
    base_hash_func = get_attribute_from_bases(bases, '__hash__')
    new_hash_func = make_hash_func(cls)
    if base_hash_func in {None, object.__hash__} or getattr(
            base_hash_func, '__code__', None) == new_hash_func.__code__:
...
class _DeprecatedFieldDescriptor:
    ...
    def __init__(
            self,
            msg: str,
            wrapped_property: property | None = None) -> None:
        self.msg = msg
    ...
    def __get__(
            self,
            obj: BaseModel | None,
            obj_type: type[BaseModel] | None = None) -> Any:
        if obj is None:
...
class _PydanticWeakRef:
    ...
    def __reduce__(self) -> tuple[Callable,
                                  tuple[weakref.ReferenceType | None]]:
