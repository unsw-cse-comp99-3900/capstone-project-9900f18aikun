...
# Check if `deprecated` is a type to prevent errors when using
...
def is_new_type(type_: type[Any]) -> bool:
    ...
    return isinstance(type_, test_new_type.__class__) and hasattr(
        type_, '__supertype__')  # type: ignore[arg-type]
...
def _check_classvar(v: type[Any] | None) -> bool:
    ...
    return v.__class__ == typing.ClassVar.__class__ and getattr(
        v, '_name', None) == 'ClassVar'
...
def parent_frame_namespace(*, parent_depth: int = 2) -> dict[str, Any] | None:
    ...
    # if f_back is None, it's the global module namespace and we don't need to
    # include it here
...
def add_module_globals(
        obj: Any, globalns: dict[str, Any] | None = None) -> dict[str, Any]:
...
def get_cls_types_namespace(cls: type[Any],
                            parent_namespace: dict[str,
...
def eval_type_lenient(value: Any,
                      globalns: dict[str,
                                     Any] | None = None,
                      localns: dict[str,
                                    Any] | None = None) -> Any:
...
def eval_type_backport(value: Any,
                       globalns: dict[str,
                                      Any] | None = None,
                       localns: dict[str,
                                     Any] | None = None) -> Any:
    ...
    try:
        ...
        try:
            ...
            raise TypeError(
                ...
                f'or install the `eval_type_backport` package.') from e
...
def is_backport_fixable_error(e: TypeError) -> bool:
    ...
    return msg.startswith(
        'unsupported operand type(s) for |: ') or "' object is not subscriptable" in msg


def get_function_type_hints(function: Callable[...,
                                               Any],
                            *,
        ...
        * Use `_make_forward_ref` instead of `typing.ForwardRef` to handle the `is_class` argument.
        ...
        if hints is None:
            ...
            if isinstance(obj, typing._allowed_types):  # type: ignore
                ...
                raise TypeError(
                    f'{obj!r} is not a module, class, method, '
                    'or function.')
        ...
        for name, value in hints.items():
            ...
            hints[name] = value
        return hints if include_extras else {
            k: typing._strip_annotations(t) for k,
            t in hints.items()}  # type: ignore
