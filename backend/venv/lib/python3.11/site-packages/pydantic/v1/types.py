...
def _registered(typ: Union[Type[T],
                           'ConstrainedNumberMeta']) -> Union[Type[T],
...
class ConstrainedNumberMeta(type):
    ...
    def __new__(cls, name: str, bases: Any,
                dct: Dict[str, Any]) -> 'ConstrainedInt':
...
def conset(item_type: Type[T],
           *,
           min_items: Optional[int] = None,
           max_items: Optional[int] = None) -> Type[Set[T]]:
...
class ConstrainedFrozenSet(frozenset):  # type: ignore
    ...
    def frozenset_length_validator(
            cls, v: 'Optional[FrozenSet[T]]') -> 'Optional[FrozenSet[T]]':
        if v is None:
            return None
...
class ConstrainedList(list):  # type: ignore
    ...
    def list_length_validator(
            cls, v: 'Optional[List[T]]') -> 'Optional[List[T]]':
        if v is None:
            return None
...
def conlist(item_type: Type[T],
            *,
            min_items: Optional[int] = None,
            max_items: Optional[int] = None,
            unique_items: bool = None) -> Type[List[T]]:
    # __args__ is needed to conform to typing generics api
    namespace = dict(
        min_items=min_items,
        max_items=max_items,
    ...
    # We use new_class to be able to deal with Generic types
    return new_class('ConstrainedListValue', (ConstrainedList,),
                     {}, lambda ns: ns.update(namespace))
...
if TYPE_CHECKING:
    ...
    class UUID1(UUID):
        ...
        def __modify_schema__(cls, field_schema: Dict[str, Any]) -> None:
            field_schema.update(
                type='string',
                format=f'uuid{cls._required_version}')
...
class PaymentCardNumber(str):
    ...
    def validate_length_for_brand(
            cls, card_number: 'PaymentCardNumber') -> 'PaymentCardNumber':
        """
