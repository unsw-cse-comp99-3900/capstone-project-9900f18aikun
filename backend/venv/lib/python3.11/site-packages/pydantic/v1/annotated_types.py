...
if sys.version_info < (3, 11):
    ...
    def is_legacy_typeddict(typeddict_cls: Type['TypedDict']) -> bool:
        return is_typeddict(typeddict_cls) and type(
            typeddict_cls).__module__ == 'typing'
...
def create_model_from_namedtuple(
    ...
    namedtuple_annotations: Dict[str, Type[Any]] = getattr(
        namedtuple_cls, '__annotations__', None) or {k: Any for k in namedtuple_cls._fields}
    field_definitions: Dict[str, Any] = {field_name: (
        field_type, Required) for field_name, field_type in namedtuple_annotations.items()}
