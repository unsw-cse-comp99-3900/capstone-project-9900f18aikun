...
if sys.version_info < (3, 8):
    ...
    def _generic_get_args(tp: Type[Any]) -> Tuple[Any, ...]:
        ...
        try:
            if tp == Tuple[()] or sys.version_info >= (
                    3, 9) and tp == tuple[()]:  # type: ignore[misc]
                return ((),)
    ...
    def get_args(tp: Type[Any]) -> Tuple[Any, ...]:
        ...
        # above)
        return _typing_get_args(tp) or getattr(
...
if sys.version_info < (3, 9):
    ...
    def convert_generics(tp: Type[Any]) -> Type[Any]:
        ...
        # `ForwardRef(arg)`
        converted = tuple(ForwardRef(arg) if isinstance(arg, str) and isinstance(
            tp, TypingGenericAlias) else convert_generics(arg) for arg in args)
...
def display_as_type(v: Type[Any]) -> str:
    if not isinstance(
            v,
            typing_base) and not isinstance(
            v,
            WithArgsTypes) and not isinstance(
                v,
            type):
        v = v.__class__
...
def resolve_annotations(
        raw_annotations: Dict[str, Type[Any]], module_name: Optional[str]) -> Dict[str, Type[Any]]:
    """
    ...
    for name, value in raw_annotations.items():
        if isinstance(value, str):
            if (3, 10) > sys.version_info >= (
                    3, 9, 8) or sys.version_info >= (3, 10, 1):
...
def is_typeddict_special(type_: Any) -> bool:
    ...
    """
    return _check_typeddict_special(
        type_) or _check_typeddict_special(get_origin(type_))
...
def is_new_type(type_: Type[Any]) -> bool:
    ...
    """
    return isinstance(
        type_, test_type.__class__) and hasattr(
        type_, '__supertype__')  # type: ignore
...
def _check_classvar(v: Optional[Type[Any]]) -> bool:
    ...
    return v.__class__ == ClassVar.__class__ and getattr(
