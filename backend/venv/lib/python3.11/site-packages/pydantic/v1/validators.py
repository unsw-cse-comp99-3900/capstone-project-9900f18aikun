...
def number_multiple_validator(v: 'Number', field: 'ModelField') -> 'Number':
    ...
    if field_type.multiple_of is not None:
        ...
        if not almost_equal_floats(
                mod,
                0.0) and not almost_equal_floats(
                mod,
                1.0):
...
def constr_upper(v: 'StrBytes', field: 'ModelField',
                 config: 'BaseConfig') -> 'StrBytes':
...
def constr_lower(v: 'StrBytes', field: 'ModelField',
                 config: 'BaseConfig') -> 'StrBytes':
    ...
    def check(self, config: Type['BaseConfig']) -> bool:
        return any(
            getattr(
                ...
                name) not in {
                None,
                self.ignored_value} for name in self.config_attr_names)
