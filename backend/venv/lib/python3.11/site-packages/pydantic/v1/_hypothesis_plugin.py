...
st.register_type_strategy(
    ...
    st.from_type(
        ipaddress.IPv4Interface) | st.from_type(
...
st.register_type_strategy(
    ...
    st.from_type(
        ipaddress.IPv4Network) | st.from_type(
...
@overload
def _registered(
        typ: pydantic.types.ConstrainedNumberMeta) -> pydantic.types.ConstrainedNumberMeta:
    pass
...
def resolve_json(cls):  # type: ignore[no-untyped-def]
    ...
    return st.builds(
        cls.inner_type.json if lenient_issubclass(
            inner_type,
            pydantic.BaseModel) else json.dumps,
        inner,
        ensure_ascii=st.booleans(),
        indent=st.none() | st.integers(
            0,
            16),
