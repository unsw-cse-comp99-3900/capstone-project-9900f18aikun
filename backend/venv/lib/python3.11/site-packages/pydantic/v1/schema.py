...
def get_field_info_schema(
        field: ModelField, schema_overrides: bool = False) -> Tuple[Dict[str, Any], bool]:
    ...
    if not field.required and field.default is not None and not is_callable_type(
            field.outer_type_):
...
def get_field_schema_validations(field: ModelField) -> Dict[str, Any]:
    ...
    if lenient_issubclass(
            field.type_,
            numeric_types) and not issubclass(
            field.type_,
            bool):
...
def get_model_name_map(
        unique_models: TypeModelSet) -> Dict[TypeModelOrEnum, str]:
    """
...
def get_flat_models_from_model(
        ...
        known_models: Optional[TypeModelSet] = None) -> TypeModelSet:
    """
...
def enum_process_schema(
        enum: Type[Enum], *, field: Optional[ModelField] = None) -> Dict[str, Any]:
    """
...
def add_field_type_to_schema(field_type: Any, schema_: Dict[str, Any]) -> None:
    ...
    for type_, t_schema in field_class_to_schema:
        ...
        # valid class
        if lenient_issubclass(field_type,
                              type_) or field_type is type_ is Pattern:
            schema_.update(t_schema)
            break


def get_schema_ref(name: str,
                   ref_prefix: Optional[str],
                   ref_template: str,
                   schema_overrides: bool) -> Dict[str,
                                                   Any]:
    if ref_prefix:
...
def field_singleton_schema(  # noqa: C901 (ignore complexity)
    ...
    *,
    ...
    if field_type is Any or field_type is object or field_type.__class__ == TypeVar or get_origin(
            field_type) is type:
...
def multitypes_literal_field_for_schema(
        values: Tuple[Any, ...], field: ModelField) -> ModelField:
    """
...
def get_annotation_from_field_info(
        ...
        field_name: str,
        validate_assignment: bool = False) -> Type[Any]:
    """
    ...
    def go(type_: Any) -> Type[Any]:
        ...
        if origin is not None:
            ...
            if issubclass(
                ...
                return conset(go(args[0]),
                              min_items=field_info.min_items,
                              max_items=field_info.max_items)

            if issubclass(
                    origin, FrozenSet) and (
                    field_info.min_items is not None or field_info.max_items is not None):
                used_constraints.update({'min_items', 'max_items'})
                return confrozenset(go(args[0]),
                                    min_items=field_info.min_items,
                                    max_items=field_info.max_items)
        ...
        if isinstance(type_, type):
            if issubclass(type_, (SecretStr, SecretBytes)):
                ...
                numeric_type = next(
                    t for t in numeric_types if issubclass(
        ...
        if attrs:
            ...
            kwargs = {
                attr_name: attr for attr_name,
                attr in (
                    (attr_name,
                     getattr(
                         field_info,
                         attr_name)) for attr_name in attrs) if attr is not None}
            if kwargs:
