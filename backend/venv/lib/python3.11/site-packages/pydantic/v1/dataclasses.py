...
@contextmanager
def set_validation(cls: Type['DataclassT'],
                   value: bool) -> Generator[Type['DataclassT'],
                                             None,
                                             None]:
...
def _dataclass_validate_values(self: 'Dataclass') -> None:
    ...
    if getattr(self, '__pydantic_has_field_info_default__', False):
        ...
        input_data = {
            k: v for k,
            v in self.__dict__.items() if not _is_field_cached_property(
                self,
                k)}
...
def _dataclass_validate_assignment_setattr(
        self: 'Dataclass', name: str, value: Any) -> None:
...
def is_builtin_dataclass(_cls: Type[Any]) -> bool:
    ...
    """
    return (dataclasses.is_dataclass(_cls) and not hasattr(_cls, '__pydantic_model__') and set(
        _cls.__dataclass_fields__).issuperset(set(getattr(_cls, '__annotations__', {}))))


def make_dataclass_validator(
        dc_cls: Type['Dataclass'],
        config: Type[BaseConfig]) -> 'CallableGenerator':
    """
