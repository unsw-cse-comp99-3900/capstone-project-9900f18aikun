...
class AnyUrl(str):
    ...
    def build(
        ...
        **_kwargs: str,
        ...
        if user or password:
            url += '@'
        url += host
        if port and ('port' not in cls.hidden_parts or cls.get_default_parts(
                parts).get('port') != port):
            url += ':' + port
        if path:
            url += path
    ...
    @classmethod
    def validate_host(
            cls, parts: 'Parts') -> Tuple[str, Optional[str], str, bool]:
    ...
    def __repr__(self) -> str:
        extra = ', '.join(
            f'{n}={getattr(self, n)!r}' for n in self.__slots__ if getattr(
                self, n) is not None)
...
class RedisDsn(AnyUrl):
    ...
    def get_default_parts(parts: 'Parts') -> 'Parts':
        return {
            'domain': 'localhost' if not (
                parts['ipv4'] or parts['ipv6']) else '',
            'port': '6379',
            'path': '/0',
        }
...
class IPvAnyAddress(_BaseAddress):
    ...
    @classmethod
    def validate(cls, value: Union[str, bytes, int]
...
class IPvAnyInterface(_BaseAddress):
    ...
    @classmethod
    def validate(
            cls, value: NetworkType) -> Union[IPv4Interface, IPv6Interface]:
        try:
