...
def root_validator(_func: Optional[AnyCallable] = None,
                   *,
                   pre: bool = False,
                   allow_reuse: bool = False,
                   skip_on_failure: bool = False) -> Union['AnyClassMethod',
...
def _prepare_validator(
        ...
        allow_reuse: bool) -> 'AnyClassMethod':
    """
        ...
        if validators:
            return {
                getattr(
                    v.func,
                    '__name__',
                    f'<No __name__: id:{id(v.func)}>'): v for v in validators}
        else:
            return None
...
def extract_validators(
        namespace: Dict[str, Any]) -> Dict[str, List[Validator]]:
...
def extract_root_validators(
        namespace: Dict[str, Any]) -> Tuple[List[AnyCallable], List[Tuple[bool, AnyCallable]]]:
    from inspect import signature
...
def inherit_validators(
        ...
        validators: 'ValidatorListDict') -> 'ValidatorListDict':
...
def _generic_validator_cls(
        ...
        sig: 'Signature',
        args: Set[str]) -> 'ValidatorCallable':
