...
class PythonTimeBuilder(BaseTimeBuilder):
    ...
    DATE_YYYY_LIMIT = Limit(
        ...
        datetime.MAXYEAR,
        YearOutOfBoundsError,
        "Year must be between {0}..{1}.".format(
            datetime.MINYEAR,
            datetime.MAXYEAR),
        year_range_check,
    )
    ...
    def build_date(
            ...
            D=None,
            DDD=None):
        YYYY, MM, DD, Www, D, DDD = cls.range_check_date(
            YYYY, MM, DD, Www, D, DDD)
    ...
    def build_time(cls, hh=None, mm=None, ss=None, tz=None):
        ...
        (hours,
         ...
         seconds,
         microseconds,
         ) = PythonTimeBuilder._distribute_microseconds(microseconds,
                                                        (hours,
                                                         minutes,
                                                         seconds),
    ...
    def build_repeating_interval(cls, R=None, Rnn=None, interval=None):
        ...
        if R is True:
            if startobject is not None:
                return cls._date_generator_unbounded(
                    startobject, durationobject)
    ...
    def range_check_interval(cls, start=None, end=None, duration=None):
        ...
        if end is not None:
            ...
            if type(end) is DateTuple:
                enddatetime = cls.build_datetime(
                    end, TupleBuilder.build_time())
            ...
            if type(start) is DateTuple:
                startdatetime = cls.build_datetime(
                    start, TupleBuilder.build_time())
    ...
    def _build_week_date(isoyear, isoweek, isoday=None):
        if isoday is None:
            return PythonTimeBuilder._iso_year_start(
                isoyear) + datetime.timedelta(weeks=isoweek - 1)
