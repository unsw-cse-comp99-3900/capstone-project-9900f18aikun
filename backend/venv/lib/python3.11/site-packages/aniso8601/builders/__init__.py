...
RepeatingIntervalTuple = namedtuple(
    "RepeatingInterval", [
        "R", "Rnn", "interval"])
        ...
        if "hh" in rangedict:
            try:
                ...
                if float(hh) > 24 and float(hh) < 25:
                    raise MidnightBoundsError(
                        "Hour 24 may only represent midnight.")
...
class TupleBuilder(BaseTimeBuilder):
    # Builder used to return the arguments as a tuple, cleans up some parse
    # methods
    @classmethod
    def build_date(
            cls,
