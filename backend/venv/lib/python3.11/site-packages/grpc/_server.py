...
def _status(
    ...
    with state.condition:
        if state.client is not _CANCELLED:
            ...
            rpc_event.call.start_server_batch(
                operations, _send_status_from_server(
                    state, _SEND_STATUS_FROM_SERVER_TOKEN), )
            state.statused = True
            _reset_per_message_state(state)
...
def _start(state: _ServerState) -> None:
    with state.lock:
        ...
        # Request a call for each registered method so we can handle any of
        # them.
