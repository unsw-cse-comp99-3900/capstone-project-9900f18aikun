...
class Swagger(object):
    ...
    def as_dict(self):
        ...
        if self.api.contact and (
                self.api.contact_email or self.api.contact_url):
            infos["contact"] = {
    ...
    def extract_tags(self, api):
        ...
        for ns in api.namespaces:
            ...
            if not ns.resources:
                continue
            # hide namespaces with all Resources hidden from Swagger
            # documentation
            if all(is_hidden(r.resource, route_doc=r.route_doc)
    ...
    def extract_resource_doc(self, resource, url, route_doc=None):
        ...
        doc["name"] = ("{}_{}".format(resource.__name__, url)
                       if route_doc else resource.__name__)

        params = merge(
            self.expected_params(doc), doc.get(
                "params", OrderedDict()))
    ...
    def expected_params(self, doc):
        ...
        for expect in doc.get("expect", []):
            if isinstance(expect, RequestParser):
                ...
                body_params = [
                    p for p in expect.__schema__ if p["in"] == "body"]
                if body_params:
        ...
        for d in doc, doc[method]:
            ...
            if "docstring" in d:
                for name, description in d["docstring"]["raises"].items():
                    for exception, handler in self.api.error_handlers.items():
                        error_responses = getattr(
                            handler, "__apidoc__", {}).get(
                            "responses", {})
                        code = (
    ...
    def serialize_definitions(self):
        return dict((name, model.__schema__)
