...
def marshal(
    ...
    """
    out, has_wildcards = _marshal(
        data, fields, envelope, skip_none, mask, ordered)
    ...
    if has_wildcards:
        ...
        for dkey, val in fields.items():
            ...
            if isinstance(val, dict):
                value = marshal(
                    data, val, skip_none=skip_none, ordered=ordered)
            else:
                field = make(val)
                ...
                if is_wildcard:
                    ...
                    def _append(k, v):
                        if skip_none and (
                                v is None or v == OrderedDict() or v == {}):
                            return
                        items.append((k, v))
        ...
        if envelope:
            out = OrderedDict([(envelope, out)]) if ordered else {
                envelope: out}
