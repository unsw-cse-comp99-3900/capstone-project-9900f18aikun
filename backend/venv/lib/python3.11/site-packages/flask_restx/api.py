...
class Api(object):
    ...
    def _register_view(self, app, resource, namespace, *urls, **kwargs):
        endpoint = kwargs.pop(
            "endpoint",
            None) or camel_to_dash(
            resource.__name__)
        resource_class_args = kwargs.pop("resource_class_args", ())
        resource_class_kwargs = kwargs.pop("resource_class_kwargs", {})

        # NOTE: 'view_functions' is cleaned up from Blueprint class in Flask
        # 1.0
    ...
    def default_endpoint(self, resource, namespace):
        ...
        if namespace is not self.default_namespace:
            endpoint = "{ns.name}_{endpoint}".format(
                ns=namespace, endpoint=endpoint)
        if endpoint in self.endpoints:
            suffix = 2
            while True:
                new_endpoint = "{base}_{suffix}".format(
                    base=endpoint, suffix=suffix)
    ...
    def _should_use_fr_error_handler(self):
        ...
        try:
            ...
            rule, _ = adapter.match(
                method=valid_route_method, return_rule=True)
    ...
    def handle_error(self, e):
        ...
        for typecheck, handler in self._own_and_child_error_handlers.items():
            ...
            # registered, unless the exception was raised from an error
            # handler."
    ...
    def as_postman(self, urlvars=False, swagger=False):
        ...
        """
        return PostmanCollectionV1(
            self, swagger=swagger).as_dict(
            urlvars=urlvars)
    ...
    def _deferred_blueprint_init(self, setup_state):
        ...
        if not setup_state.first_registration:
            raise ValueError(
                "flask-restx blueprints can only be registered once.")
    ...
    def unauthorized(self, response):
        ...
        if self.serve_challenge_on_401:
            realm = current_app.config.get(
                "HTTP_BASIC_AUTH_REALM", "flask-restx")
...
def mask_parse_error_handler(error):
    ...
    return {"message": "Mask parse error: {0}".format(
