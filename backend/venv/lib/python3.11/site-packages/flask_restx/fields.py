...
class Nested(Raw):
    ...
    def clone(self, mask=None):
        ...
        if mask:
            model = mask.apply(
                model.resolved if hasattr(
                    model, "resolved") else model)
...
class List(Raw):
    ...
    def format(self, value):
        ...
        is_nested = isinstance(
            self.container,
            Nested) or type(
            self.container) is Raw

        def is_attr(val):
            return self.container.attribute and hasattr(
                val, self.container.attribute)
...
class Polymorph(Nested):
    ...
    def output(self, key, obj, ordered=False, **kwargs):
        ...
        candidates = [
            fields for cls, fields in self.mapping.items() if isinstance(value,             fields for cls, fields in self.mapping.items() if)]

        if len(candidates) <= 0:
            raise ValueError("Unknown class: " + value.__class__.__name__)
        elif len(candidates) > 1:
            raise ValueError(
                "Unable to determine a candidate for: " +
    ...
    def resolve_ancestor(self, models):
        ...
        if len(candidates) != 1:
            ...
            raise ValueError(
                "Unable to determine the common ancestor for: " +
                ", ".join(field_names))
