...
class EMA(object):
    ...
    def __call__(self, x=None):
        ...
        if x is not None:
            ...
            self.calls += 1
        return self.last / \
            (1 - beta ** self.calls) if self.calls else self.last
...
class tqdm(Comparable):
    ...
    def format_meter(
        ...
        try:
            eta_dt = (
                datetime.now() +
                timedelta(
                    seconds=remaining) if rate and total else datetime.fromtimestamp(
                    0,
                    timezone.utc))
    ...
    def __len__(self):
        return (
            self.total if self.iterable is None else self.iterable.shape[0] if hasattr(
                self.iterable,
                "shape") else len(
                self.iterable) if hasattr(
                self.iterable,
                "__len__") else self.iterable.__length_hint__() if hasattr(
                    self.iterable,
                    "__length_hint__") else getattr(
                        self,
                        "total",
                None))
    ...
    def __contains__(self, item):
        ...
        return contains(
            item) if contains is not None else item in self.__iter__()
