...
def build(
    ...
    for discovery_url in _discovery_service_uri_options(
        discoveryServiceUrl, version):
...
def build_from_document(
    ...
    if "rootUrl" not in service and isinstance(
        http, (HttpMock, HttpMockSequence)):
        logger.error(
    ...
    if http is None:
        ...
        if scopes and not developerKey:
            ...
            # Check google-api-core >= 2.18.0 if credentials' universe !=
            # "googleapis.com".
        ...
        if (
    credentials and isinstance(
        credentials,
        service_account.Credentials) and always_use_jwt_access and hasattr(
            service_account.Credentials,
             "with_always_use_jwt_access")):
            credentials = credentials.with_always_use_jwt_access(
...
if isinstance(value, type("")) or type(value) == type(""):
            return value
        else:
            return str(value)
    ...
    "Got an unexpected keyword argument {}".format(name))
        ...
        for name in parameters.required_params:
            if name not in kwargs:
                ...
                # page token parameter (cf. drive.changes.watch vs.
                # drive.changes.list)
...
if key in parameters.repeated_params and isinstance(value, type([])):
        ...
        if media_filename:
            ...
            if isinstance(media_filename, str):
                ...
                if not mimeparse.best_match(
    [media_mime_type], ",".join(accept)):
                    raise UnacceptableMimeTypeError(media_mime_type)
                media_upload = MediaFileUpload(
    media_filename, mimetype=media_mime_type)
...
def _findPageTokenName(fields):
    ...
    """
    return next(
    (tokenName for tokenName in _PAGE_TOKEN_NAMES if tokenName in fields),
     None )
