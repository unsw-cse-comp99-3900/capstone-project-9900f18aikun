...
class _StreamingResponseIterator(Generic[P], grpc.Call):
    def __init__(self, wrapped, prefetch_first_result=True):
        ...
        # to retrieve the first result, in order to fail, in order to trigger a
        # retry.
        try:
...
def _create_composite_credentials(
    ...
    if ssl_credentials:
        ...
        # See
        # https://grpc.github.io/grpc/python/grpc.html#grpc.composite_channel_credentials
...
def create_channel(
    ...
    **kwargs,
    ...
    if ssl_credentials and attempt_direct_path:
        raise ValueError(
            "Using ssl_credentials with Direct Path is not supported")
...
def _modify_target_for_direct_path(target: str) -> str:
    ...
    if direct_path_separator not in target:
        ...
        # Modify the target to use Direct Path by adding the `google-c2p:///`
        # prefix
...
class _CallableStub(object):
    ...
    def __call__(
        ...
        raise ValueError(
            'Method stub for "{}" has no response.'.format(
                self._method))
