...
async def retry_target_stream(
    ...
    **kwargs,
    ...
    # keep track of retryable exceptions we encounter to pass in to
    # exception_factory
    ...
    for sleep in sleep_generator:
        ...
        try:
            ...
            try:
                ...
                # unwrap the awaitable so we can work with the generator
                # directly
                target_output = await target_output  # type: ignore
            except TypeError:
                # was not awaitable, continue
                pass
            target_iterator = cast(
                AsyncIterable["_Y"],
                target_output).__aiter__()

            if target_is_generator is None:
                # Check if target supports generator features (asend, athrow,
                # aclose)
                target_is_generator = bool(
                    getattr(target_iterator, "asend", None))
