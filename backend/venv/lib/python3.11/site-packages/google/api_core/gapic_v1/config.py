...
def _retry_from_retry_config(
        retry_params,
        retry_codes,
        retry_impl=retry.Retry):
    ...
    return retry_impl(
        retry.if_exception_type(
            *
            exception_classes),
        initial=(
            retry_params["initial_retry_delay_millis"] /
            _MILLIS_PER_SECOND),
        maximum=(
            retry_params["max_retry_delay_millis"] /
            _MILLIS_PER_SECOND),
        multiplier=retry_params["retry_delay_multiplier"],
        deadline=retry_params["total_timeout_millis"] /
        _MILLIS_PER_SECOND,
    )
...
def _timeout_from_retry_config(retry_params):
    ...
    return timeout.ExponentialTimeout(
        initial=(
            retry_params["initial_rpc_timeout_millis"] /
            _MILLIS_PER_SECOND),
        maximum=(
            retry_params["max_rpc_timeout_millis"] /
            _MILLIS_PER_SECOND),
        multiplier=retry_params["rpc_timeout_multiplier"],
        deadline=(
            retry_params["total_timeout_millis"] /
            _MILLIS_PER_SECOND),
    )
...
def parse_method_configs(interface_config, retry_impl=retry.Retry):
    ...
    for method_name, method_params in interface_config.get(
            "methods", {}).items():
