    ...
    def _RegularMessageToJsonObject(self, message, js):
        ...
        try:
            ...
            raise SerializeToJsonError(
                'Failed to serialize {0} field: {1}.'.format(
                    field.name, e)) from e
...
class _Parser(object):
                ...
                if _IsMapEntry(field):
                    ...
                    if field.cpp_type == descriptor.FieldDescriptor.CPPTYPE_MESSAGE:
                        ...
                        for index, item in enumerate(value):
                            ...
                            if (item is None and sub_message.DESCRIPTOR.full_name !=
                            ...
                            if item is None:
                                raise ParseError(
                                    ...
                                    ' in a repeated field at {0}.{1}[{2}]'.format(
                                        path, name, index))
                            getattr(
                                message, field.name).append(
                                _ConvertScalarFieldValue(
                                    item, field, '{0}.{1}[{2}]'.format(
                                        path, name, index)))
                    ...
                    if field.is_extension:
                        ...
                        setattr(
                            message, field.name, _ConvertScalarFieldValue(
                                value, field, '{0}.{1}'.format(
                                    path, name)))
    ...
    def _ConvertListValueMessage(self, value, message, path):
        ...
        if not isinstance(value, list):
            raise ParseError(
                'ListValue must be in [] which is {0} at {1}'.format(
                    value, path))
    ...
    def _ConvertStructMessage(self, value, message, path):
        ...
        if not isinstance(value, dict):
            raise ParseError(
                'Struct must be in a dict which is {0} at {1}'.format(
                    value, path))
    ...
    def _ConvertWrapperMessage(self, value, message, path):
        ...
        setattr(message, 'value', _ConvertScalarFieldValue(
            value, field, path='{0}.value'.format(path)))
