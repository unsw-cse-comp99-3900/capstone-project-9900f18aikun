...
def _ParameterDecorator(naming_type, testcases):
    ...
    def _Apply(obj):
        if isinstance(obj, type):
            _ModifyClass(
                obj,
                list(testcases) if not isinstance(
                    testcases,
                    collections_abc.Sequence) else testcases,
                naming_type)
            return obj
        else:
...
def _UpdateClassDictForParamTestCase(dct, id_suffix, name, iterator):
    ...
    for idx, func in enumerate(iterator):
        assert callable(
            func), 'Test generators must yield callables, got %r' % (func,)
