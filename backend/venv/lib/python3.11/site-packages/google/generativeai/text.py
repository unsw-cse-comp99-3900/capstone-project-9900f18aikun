...
def _generate_response(
    ...
    response["filters"] = palm_safety_types.convert_filters_to_enums(
        response["filters"])
    response["safety_feedback"] = palm_safety_types.convert_safety_feedback_to_enums(
        response["safety_feedback"])
    response["candidates"] = palm_safety_types.convert_candidate_enums(
        response["candidates"])
...
def count_text_tokens(
    ...
    result = client.count_text_tokens(
        protos.CountTextTokensRequest(
            model=base_model, prompt={
                "text": prompt}), **request_options, )
...
def generate_embeddings(
    ...
    if isinstance(text, str):
        ...
        for batch in _batched(text, EMBEDDING_MAX_BATCH_SIZE):
            # TODO(markdaoust): This could use an option for returning an
            # iterator or wait-bar.
            embedding_request = protos.BatchEmbedTextRequest(
                model=model, texts=batch)
            embedding_response = client.batch_embed_text(
                embedding_request,
                **request_options,
            )
            embedding_dict = type(embedding_response).to_dict(
                embedding_response)
            result["embedding"].extend(e["value"]
                                       for e in embedding_dict["embeddings"])
        return result
