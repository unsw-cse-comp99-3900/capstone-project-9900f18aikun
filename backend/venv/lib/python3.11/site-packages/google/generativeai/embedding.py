...
def embed_content(
    ...
    if title and to_task_type(
            task_type) is not EmbeddingTaskType.RETRIEVAL_DOCUMENT:
        raise ValueError(
    ...
    if isinstance(
            content, Iterable) and not isinstance(
            content, (str, Mapping)):
        result = {"embedding": []}
        requests = (
        ...
        for batch in _batched(requests, EMBEDDING_MAX_BATCH_SIZE):
            embedding_request = protos.BatchEmbedContentsRequest(
            ...
            embedding_dict = type(embedding_response).to_dict(
                embedding_response)
            result["embedding"].extend(e["values"]
