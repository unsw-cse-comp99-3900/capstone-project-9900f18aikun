...
def _generate_prompts(
    ...
    for prompt_num, prompt in enumerate(prompts):
        for input_num, prompt_vars in enumerate(normalized_inputs):
            # Perform keyword substitution on the prompt based on
            # `prompt_vars`.
...
class LLMFunction(
    ...
    def __init__(self, outputs_ipython_display_fn: Callable[[
            llmfn_outputs.LLMFnOutputs], None] | None = None, ):
        """Constructor.
    ...
    def __call__(
        ...
        return llmfn_outputs.LLMFnOutputs(
            outputs=outputs,
            ipython_display_fn=self._outputs_ipython_display_fn)
    ...
    def add_post_process_add_fn(
        ...
        self._post_process_cmds.append(
            llmfn_post_process_cmds.LLMFnPostProcessAddCommand(
                name=name, fn=fn))
        return self
...
class LLMFunctionImpl(LLMFunction):
    ...
    def _run_post_processing_cmds(
        ...
        for cmd in self._post_process_cmds:
            try:
                if isinstance(
                        cmd,
                        llmfn_post_process_cmds.LLMFnImplPostProcessCommand):
    ...
    def _call_impl(
        ...
        for info in _generate_prompts(prompts=self._prompts, inputs=inputs):
            ...
            for result_num, text_result in enumerate(
...
class LLMCompareFunction(LLMFunction):
    ...
    def _call_impl(
        ...
        for lhs_entry, rhs_entry in zip(lhs_results, rhs_results):
            ...
            # minimum of the two.
            num_output_rows = min(len(lhs_entry.output_rows),
                                  len(rhs_entry.output_rows))
            ...
            for result_num, lhs_and_rhs_output_row in enumerate(
                ...
                # Add the prompt vars.
                output_row.add(
                    llmfn_outputs.ColumnNames.PROMPT_VARS,
                    lhs_entry.prompt_vars)
