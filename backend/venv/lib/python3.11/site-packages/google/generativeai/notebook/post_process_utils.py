...
class ParsedPostProcessExpr(abc.ABC):
    ...
    def add_to_llm_function(
            self,
            llm_fn: llm_function.LLMFunction) -> llm_function.LLMFunction:
...
class _ParsedPostProcessAddExpr(
    ...
    def __call__(
            self, rows: Sequence[llmfn_output_row.LLMFnOutputRowView]) -> Sequence[Any]:
...
class _ParsedPostProcessReplaceExpr(
    ...
    def __call__(
            self, rows: Sequence[llmfn_output_row.LLMFnOutputRowView]) -> Sequence[str]:
        return [self._fn(row.result_value()) for row in rows]
...
def _resolve_one_post_processing_expression(
    ...
    for part_num, part in enumerate(token_parts):
        ...
        if part not in current_module_vars:
            raise PostProcessParseError('Unable to resolve "{}"'.format(
                ".".join(token_parts[: part_num + 1])))
...
def resolve_post_processing_tokens(
    ...
    for expression in tokens:
        expr_name, expr_value = _resolve_one_post_processing_expression(
            expression)
        if isinstance(expr_value, ParsedPostProcessExpr):
            results.append(expr_value)
        elif isinstance(expr_value, Callable):
            # By default, assume that an undecorated function is an "add"
            # function.
            results.append(
                _ParsedPostProcessAddExpr(
                    name=expr_name,
                    fn=expr_value))
        else:
