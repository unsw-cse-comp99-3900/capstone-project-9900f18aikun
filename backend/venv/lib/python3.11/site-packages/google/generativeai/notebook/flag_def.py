...
class _SingleValueStoreAction(argparse.Action):
    ...
    def __call__(
        ...
        try:
            ...
            raise argparse.ArgumentError(
                self, 'Error with value "{}", got {}: {}'.format(
                    values[0], _get_type_name(
                        type(e)), e), )
...
class EnumFlagDef(SingleValueFlagDef):
    ...
    def __init__(self, *args, enum_type: type[enum.Enum], **kwargs):
        ...
        # These properties are set by "enum_type" so don"t let the caller set
        # them.
        ...
        if "choices" in kwargs:
            ...
            for x in kwargs["choices"]:
                try:
                    ...
                    raise ValueError(
                        'Invalid value in "choices": "{}"'.format(x)) from None
        else:
...
class BooleanFlagDef(FlagDef):
    ...
    def _do_additional_validation(self) -> None:
        ...
        if self.parse_to_dest_type_fn is not None:
            raise ValueError(
                "parse_to_dest_type_fn cannot be set for BooleanFlagDef")
        if self.choices is not None:
