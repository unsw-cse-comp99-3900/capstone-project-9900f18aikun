    ...
    for inputs in source.to_normalized_inputs():
        for keyword in placeholders:
            if keyword not in inputs:
                raise ValueError(
                    'Placeholder "{}" not found in input'.format(keyword))
...
def _add_model_flags(
    ...
    flag_def.SingleValueFlagDef(
        name="model", short_name="m", default_value=None, help_msg=(
            "The name of the model to use. If not provided, a default model will"
            " be used."), ).add_argument_to_parser(parser)
...
def _create_parser(
    ...
    _create_compile_parser(subparsers.add_parser(
        parsed_args_lib.CommandName.COMPILE_CMD.value))
...
class CmdLineParser:
    ...
    def _split_post_processing_tokens(
        ...
        split_tokens.append(tokens[start_idx:]
    ...
    def _tokenize_line(
        ...
        if not first_token[0].isalpha() and first_token not in [
                "-h", "--help"]:
    ...
    def parse_line(
        ...
        if parsed_args.cmd == parsed_args_lib.CommandName.COMPARE_CMD:
            ...
            parsed_args = self._get_parsed_args_from_cmd_line_tokens(
                tokens=tokens, placeholders=frozenset(
                    lhs_fn.get_placeholders()).union(
