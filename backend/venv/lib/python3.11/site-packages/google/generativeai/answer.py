...
GroundingPassageOptions = (Union[protos.GroundingPassage,
                                 tuple[str,
...
def _make_grounding_passages(
        source: GroundingPassagesOptions) -> protos.GroundingPassages:
    """
    ...
    for n, data in enumerate(source):
        if isinstance(data, protos.GroundingPassage):
            ...
            passages.append(
                {"id": id, "content": content_types.to_content(content)})
        else:
            passages.append(
                {"id": str(n), "content": content_types.to_content(data)})

    return protos.GroundingPassages(passages=passages)


SourceNameType = Union[str,
