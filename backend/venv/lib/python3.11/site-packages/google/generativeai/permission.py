...
def _validate_resource_name(x: str, resource_type: str) -> None:
    if resource_type == "corpora":
        if not retriever_types.valid_name(x):
            raise ValueError(
                retriever_types.NAME_ERROR_MSG.format(
                    length=len(x), name=x))

    elif resource_type == "tunedModels":
        if not model_types.valid_tuned_model_name(x):
            raise ValueError(
                model_types.TUNED_MODEL_NAME_ERROR_MSG.format(
                    length=len(x), name=x))
...
def _validate_permission_id(x: str) -> None:
    if not permission_types.valid_id(x):
        raise ValueError(
            permission_types.INVALID_PERMISSION_ID_MSG.format(
                permission_id=x))


def _get_valid_name_components(name: str) -> str:
    # name is of the format:
    # resource_type/resource_name/permissions/permission_id
...
def _construct_name(
    ...
    # resource_name is the name of the supported resource (corpus or
    # tunedModel as of now) for which the permission is being created.
    if not name:
        # if name is not provided, then try to construct name via provided
        ...
        if resource_type:
            ...
            # if resource_type is not provided, then try to infer it from
            # resource_name.
        ...
        if isinstance(
                permission_id,
                int) or "permissions/" not in permission_id:
