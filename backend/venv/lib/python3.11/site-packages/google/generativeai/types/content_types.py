...
# For generate_content, we're not guessing roles for
# [[parts],[parts],[parts]] yet.
...
def to_contents(contents: ContentsType) -> list[protos.Content]:
    ...
    if isinstance(
            contents, Iterable) and not isinstance(
            contents, (str, Mapping)):
        try:
    ...
    *,
    ...
    for name, param in defaults.items():
        if param.kind in (
            ...
            # it will not try to auto-infer the type based on the default
            # value.
...
def _rename_schema_fields(schema):
    ...
    if properties is not None:
        schema["properties"] = {
            k: _rename_schema_fields(v) for k,
...
def _encode_fd(fd: FunctionDeclaration |
               protos.FunctionDeclaration) -> protos.FunctionDeclaration:
...
class Tool:
    ...
    def __init__(
        ...
        self._proto = protos.Tool(
            function_declarations=[
                _encode_fd(fd) for fd in self._function_declarations])

    @property
    def function_declarations(
            self) -> list[FunctionDeclaration | protos.FunctionDeclaration]:
    ...
    def __call__(
            self,
            fc: protos.FunctionCall) -> protos.FunctionResponse | None:
        declaration = self[fc]
...
ToolType = Union[Tool, protos.Tool, ToolDict,
                 Iterable[FunctionDeclarationType], FunctionDeclarationType]
...
def _make_tools(tools: ToolsType) -> list[Tool]:
    if isinstance(tools, Iterable) and not isinstance(tools, Mapping):
        ...
        if len(tools) > 1 and all(
                len(t.function_declarations) == 1 for t in tools):
...
def to_function_library(lib: FunctionLibraryType |
                        None) -> FunctionLibrary | None:
    if lib is None:
        return lib
...
def to_function_calling_mode(
        x: FunctionCallingModeType) -> FunctionCallingMode:
    if isinstance(x, str):
        x = x.lower()
...
FunctionCallingConfigType = Union[FunctionCallingModeType,
                                  FunctionCallingConfigDict, protos.FunctionCallingConfig]


def to_function_calling_config(
        obj: FunctionCallingConfigType) -> protos.FunctionCallingConfig:
