...
UpdateChunkOptions = Union[protos.UpdateChunkRequest,
                           Mapping[str, Any], tuple[str, Any]]

BatchUpdateChunksOptions = Union[protos.BatchUpdateChunksRequest,
                                 Iterable[UpdateChunkOptions]]
    ...
    def _to_proto(self):
        ...
        if isinstance(self.value, str):
            ...
            if isinstance(self.value, Mapping):
                ...
                kwargs["string_list_value"] = protos.StringList(
                    values=self.value)
...
class Corpus:
    ...
    def create_document(
        ...
        if name is None:
            ...
            raise ValueError(
                NAME_ERROR_MSG.format(
                    length=len(name),
                    name=name))

        request = protos.CreateDocumentRequest(
            parent=self.name, document=document)
    ...
    def update(
        ...
        request = protos.UpdateCorpusRequest(
            corpus=self.to_dict(), update_mask=field_mask)
    ...
    def query(
        ...
        # Create a RelevantChunk object for each chunk listed in
...
class Document(abc.ABC):
    ...
    def _make_batch_create_chunk_request(
        ...
        for i, chunk in enumerate(chunks):
            ...
            requests.append(
                protos.CreateChunkRequest(
                    parent=self.name,
                    chunk=chunk))

        return protos.BatchCreateChunksRequest(
            parent=self.name, requests=requests)
    ...
    def batch_update_chunks(
        ...
        if isinstance(chunks, Mapping):
            ...
            for key, value in chunks.items():
                ...
                # When handling updates, use to the _to_proto result of the
                # custom_metadata
            ...
            request = protos.BatchUpdateChunksRequest(
                parent=self.name, requests=_requests)
        ...
        if isinstance(chunks, Iterable) and not isinstance(chunks, Mapping):
            for chunk in chunks:
                if isinstance(chunk, protos.UpdateChunkRequest):
                    ...
                    # First element is name of chunk, second element contains
                    # updates
...
class Chunk(abc.ABC):
    ...
    def __init__(
        ...
        if custom_metadata is None:
            ...
            self.custom_metadata = [
                make_custom_metadata(cm) for cm in custom_metadata]
    ...
    def update(
        ...
        request = protos.UpdateChunkRequest(
            chunk=self.to_dict(), update_mask=field_mask)
