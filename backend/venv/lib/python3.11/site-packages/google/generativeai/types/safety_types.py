...
SafetySettingOptions = Union[HarmBlockThresholdOptions,
                             EasySafetySetting, Iterable[LooseSafetySettingDict], None]
...
def _expand_block_threshold(block_threshold: HarmBlockThresholdOptions):
    ...
    return {
        category: block_threshold for category in set(
            _HARM_CATEGORIES.values())}


def to_easy_safety_dict(
        settings: SafetySettingOptions) -> EasySafetySettingDict:
    if settings is None:
        return {}
    ...
    if isinstance(settings, Mapping):
        return {to_harm_category(key): to_block_threshold(value)
