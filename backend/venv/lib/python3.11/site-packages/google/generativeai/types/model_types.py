...
def decode_tuned_model(tuned_model: protos.TunedModel |
                       dict["str", Any]) -> TunedModel:
    if isinstance(tuned_model, protos.TunedModel):
        tuned_model = type(tuned_model).to_dict(
            tuned_model)  # pytype: disable=attribute-error
...
TuningExampleOptions = Union[TuningExampleDict,
                             protos.TuningExample, tuple[str, str], list[str]]
...
def encode_tuning_example(
        example: TuningExampleOptions,
        input_key,
        output_key):
    if isinstance(example, protos.TuningExample):
        ...
        example = protos.TuningExample(
            text_input=example[input_key],
...
AnyModelNameOptions = Union[str, Model,
                            protos.Model, TunedModel, protos.TunedModel]
