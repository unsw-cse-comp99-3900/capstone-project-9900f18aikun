...
def _get_end2end_headers(response):
    ...
    return [
        header for header in list(
            response.keys()) if header not in hopbyhop]
...
def _normalize_headers(headers):
    return dict([(_convert_byte_str(key).lower(), NORMALIZE_SPACE.sub(
        _convert_byte_str(value), " ").strip(),) for (key, value) in headers.items()])
...
def _updateCache(request_headers, response_headers, content, cache, cachekey):
    if cachekey:
        ...
        if "no-store" in cc or "no-store" in cc_response:
            ...
            header_str = re.sub("\r(?!\n)|(?<!\r)\n", "\r\n", header_str)
            text = b"".join([status_header.encode("utf-8"),
    ...
    dig = _md5(
        ("%s:%s" %
         (time.ctime(), [
             "0123456789"[
                 random.randrange(
                     0, 9)] for i in range(20)])).encode("utf-8")).hexdigest()
    return dig[:16]
        ...
        base64.b64encode(
            _sha(
                ("%s%s%s" %
                 (cnonce,
                  iso_now,
                  password)).encode("utf-8")).digest()).strip().decode("utf-8"))
...
class Http(object):
    ...
    def _request(
        ...
        if self.follow_all_redirects or method in self.safe_methods or response.status in (
                303,
                308):
        ...
        try:
            ...
            if self.force_exception_to_status_code:
                if isinstance(e, HttpLib2ErrorWithResponse):
                    ...
                    response = Response(
                        {"content-type": "text/plain", "status": "408", "content-length": len(content), })
                    response.reason = "Request Timeout"
                else:
                    content = str(e).encode("utf-8")
                    response = Response(
                        {"content-type": "text/plain", "status": "400", "content-length": len(content), })
