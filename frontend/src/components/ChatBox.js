...
export const ChatBox = ({ change, setChange }) => {
  ...
  useEffect(() => {
    localStorage.setItem('expressBookMessages', JSON.stringify(expressBookMessages));
  }, [expressBookMessages]);
  const toggleChatBox = () => {
  ...
  const toggleRoomType = () => {
    if (mode === 'ExpressBook') {
      ...
      const message = { 
        text: `You switched to ${newRoomType === "hotdesk" ? "hotdesk" : "meeting room"}!`, 
        sender: "bot", 
        timestamp: new Date() 
      };
  ...
  const handleBookRoom = async () => {
    try {
      ...
      const bookingInfo = {
        ...
        start_time: formatTime(selectedRoom.start_time),
  ...
  const handleRequestRoom = async (roomName) => {
    try {
      ...
      const selectedRoom = latestRoomData.find(room => room.name === roomName);
                        ...
                        <div className="message-text">
                          {msg.text.split('\n').map((line, i) => {
                            if (line.includes('"exchange.png"')) {
                              const [before, after] = line.split('"exchange.png"');
                              return (
                                <p key={i}>
                                  {before}
                                  <img 
                            ...
                            if (line.includes('[Select Room]')) {
                              ...
                              const roomName = line.match(/\(request:(.*?)\)/)[1];
                              return (
                                <button 
                                  key={i} 
                            ...
                            } else if (line.includes('Press the exchange button')) {
                              return (
                                <p key={i}>
                                  {line.replace('exchange button', 'exchange button ')}
                                  <img 
                                    src="/chat_box/exchange.png" 
                                    alt="Exchange" 
